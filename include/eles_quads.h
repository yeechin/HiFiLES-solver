...
public:
  ...
  /*! set shape */
  //void set_shape(arrayt<int> &in_n_spts_per_ele);
  ...
  void set_loc_1d_spts(arrayt<double> &loc_1d_spts, int in_n_1d_spts);
  ...
  double compute_inter_detjac_inters_cubpts(int in_inter, arrayt<double> d_pos);

  /*! evaluate nodal basis */
  double eval_nodal_basis(int in_index, arrayt<double> in_loc);

  /*! evaluate nodal basis restart*/
  double eval_nodal_basis_restart(int in_index, arrayt<double> in_loc);

  /*! evaluate derivative of nodal basis */
  double eval_d_nodal_basis(int in_index, int in_cpnt, arrayt<double> in_loc);

  /*! evaluate divergence of vcjh basis */
  double eval_div_vcjh_basis(int in_index, arrayt<double>& loc);

  void fill_opp_3(arrayt<double>& opp_3);

  /*! evaluate nodal shape basis */
  double eval_nodal_s_basis(int in_index, arrayt<double> in_loc, int in_n_spts);

  /*! evaluate derivative of nodal shape basis */
  void eval_d_nodal_s_basis(arrayt<double> &d_nodal_s_basis, arrayt<double> in_loc, int in_n_spts);
  ...
  /*! setup the concentration arrayt required for concentration method for shock capturing */
  void set_concentration_arrayt(void);

  /*! set filter arrayt */
  void set_filter_arrayt(void);

  /*! exponential filter */
  double exponential_filter(int, int);

  /*! Evaluate 2D Legendre Basis */
  double eval_legendre_basis_2D_hierarchical(int, arrayt<double>, int in_order);
...
protected:
  ...
  arrayt<double> loc_1d_upts;

  /*! location of solution points in standard interval (tensor product elements only)*/
  arrayt<double> loc_1d_upts_rest;

  /*! element edge lengths for h_ref calculation */
  arrayt<double> length;
