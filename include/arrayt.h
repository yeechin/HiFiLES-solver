...
public:
  ...
  arrayt(int in_dim_0, int in_dim_1=1, int in_dim_2=1, int in_dim_3=1);

  // copy constructor

  arrayt(const arrayt<T>& in_arrayt);

  // assignment

  arrayt<T>& operator=(const arrayt<T>& in_arrayt);
  ...
  // method to get maximum value of arrayt

  T get_max(void);

  // method to get minimum value of arrayt
  ...
  /*! Initialize arrayt to zero - Valid for numeric data types (int, float, double) */
  void initialize_to_zero();

  /*! Initialize arrayt to given value */
...
template <typename T>
arrayt<T>::arrayt(int in_dim_0, int in_dim_1, int in_dim_2, int in_dim_3)
{
  dim_0=in_dim_0;
  dim_1=in_dim_1;
  dim_2=in_dim_2;
...
template <typename T>
arrayt<T>::arrayt(const arrayt<T>& in_arrayt)
{
  int i;

  dim_0=in_arrayt.dim_0;
  dim_1=in_arrayt.dim_1;
  dim_2=in_arrayt.dim_2;
  dim_3=in_arrayt.dim_3;

  cpu_data = new T[dim_0*dim_1*dim_2*dim_3];

  for(i=0; i<dim_0*dim_1*dim_2*dim_3; i++)
    {
      cpu_data[i]=in_arrayt.cpu_data[i];
    }
}
...
template <typename T>
arrayt<T>& arrayt<T>::operator=(const arrayt<T>& in_arrayt)
{
  int i;
...
template <typename T>
void arrayt<T>::setup(int in_dim_0, int in_dim_1, int in_dim_2, int in_dim_3)
{
  delete[] cpu_data;
...
template <typename T>
T* arrayt<T>::get_ptr_cpu(void)
{
  if(cpu_flag==1)
    return cpu_data;
  else
    FatalError("CPU arrayt does not exist");
}
...
T* arrayt<T>::get_ptr_gpu(void)
{
  if(gpu_flag==1)
  ...
  else
    {
      ...
      FatalError("GPU arrayt does not exist");
    }
}
...
template <typename T>
T* arrayt<T>::get_ptr_cpu(int in_pos_0, int in_pos_1, int in_pos_2, int in_pos_3)
{
  if(cpu_flag==1)
    return cpu_data+in_pos_0+(dim_0*in_pos_1)+(dim_0*dim_1*in_pos_2)+(dim_0*dim_1*dim_2*in_pos_3); // column major with matrix indexing
  else
...
template <typename T>
T* arrayt<T>::get_ptr_gpu(int in_pos_0, int in_pos_1, int in_pos_2, int in_pos_3)
{
  if(gpu_flag==1)
...
template <typename T>
int arrayt<T>::get_dim(int in_dim)
{
  if(in_dim==0)
    {
      return dim_0;
...
// method to calculate maximum value of arrayt
// Template specialization
template <typename T>
T arrayt<T>::get_max(void)
{
  int i;
  T max = 0;
...
// method to calculate minimum value of arrayt
// Template specialization
template <typename T>
T arrayt<T>::get_min(void)
{
  int i;
  T min = 1e16;
...
void arrayt<T>::print(void)
{
  if(dim_3==1)
    ...
    {
      cout << "ERROR: Can only print an arrayt of dimension three or less ...." << endl;
    }
}

#ifdef _GPU
template <typename T>
void arrayt<T>::check_cuda_error(const char *message, const char *filename, const int lineno)
{
...
template <typename T>
void arrayt<T>::initialize_to_zero()
{
...
// Initialize arrayt to given value
template <typename T>
void arrayt<T>::initialize_to_value(const T val)
{
  for(int i=0; i<dim_0*dim_1*dim_2*dim_3; i++)
  {
    cpu_data[i]=val;
