...
public:
  ...
  void write_restart_mesh(ofstream& restart_file);
	...
	/*! move wall distance arrayt to from cpu to gpu */
	void mv_wall_distance_cpu_gpu(void);

  /*! move wall distance magnitude arrayt to from cpu to gpu */
  ...
  /*! set shape node */
  void set_shape_node(int in_spt, int in_ele, arrayt<double>& in_pos);

  /*! Set new position of shape node in dynamic domain */
  void set_dynamic_shape_node(int in_spt, int in_ele, arrayt<double> &in_pos);
  ...
  /*! get a pointer to gradient of discontinuous solution at a flux point */
  double* get_normal_disu_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter, int in_field, int in_ele, arrayt<double> temp_loc, double temp_pos[3]);
  ...
  void calc_pos_ppts(int in_ele, arrayt<double>& out_pos_ppts);
  ...
  arrayt<int> get_connectivity_plot();

  /*! calculate solution at the plot points */
  void calc_disu_ppts(int in_ele, arrayt<double>& out_disu_ppts);

  /*! calculate gradient of solution at the plot points */
  void calc_grad_disu_ppts(int in_ele, arrayt<double>& out_grad_disu_ppts);

  /*! calculate sensor at the plot points */
  void calc_sensor_ppts(int in_ele, arrayt<double>& out_sensor_ppts);

  /*! calculate AV-co-efficients at the plot points */
  void calc_epsilon_ppts(int in_ele, arrayt<double>& out_epsilon_ppts);

  /*! calculate time-averaged diagnostic fields at the plot points */
  void calc_time_average_ppts(int in_ele, arrayt<double>& out_disu_average_ppts);

  /*! calculate diagnostic fields at the plot points */
  void calc_diagnostic_fields_ppts(int in_ele, arrayt<double>& in_disu_ppts, arrayt<double>& in_grad_disu_ppts, arrayt<double>& in_sensor_ppts, arrayt<double> &in_epsilon_ppts, arrayt<double>& out_diag_field_ppts, double& time);

  /*! calculate position of a solution point */
  void calc_pos_upt(int in_upt, int in_ele, arrayt<double>& out_pos);

  /*! get physical position of a flux point */
  void calc_pos_fpt(int in_fpt, int in_ele, arrayt<double>& out_pos);
  ...
  void set_transforms_vol_cubpts(void);
	...
	void calc_wall_distance(int n_seg_noslip_inters, int n_tri_noslip_inters, int n_quad_noslip_inters, arrayt< arrayt<double> > loc_noslip_bdy);

	/*! Calculate distance of solution points to no-slip wall in parallel */
	void calc_wall_distance_parallel(arrayt<int> n_seg_noslip_inters, arrayt<int> n_tri_noslip_inters, arrayt<int> n_quad_noslip_inters, arrayt< arrayt<double> > loc_noslip_bdy_global, int nproc);

  /*! calculate position */
  void calc_pos(arrayt<double> in_loc, int in_ele, arrayt<double>& out_pos);

  /*! calculate derivative of position */
  void calc_d_pos(arrayt<double> in_loc, int in_ele, arrayt<double>& out_d_pos);

  /*! calculate derivative of position at a solution point (using pre-computed gradients) */
  void calc_d_pos_upt(int in_upt, int in_ele, arrayt<double>& out_d_pos);

  /*! calculate derivative of position at a flux point (using pre-computed gradients) */
  void calc_d_pos_fpt(int in_fpt, int in_ele, arrayt<double>& out_d_pos);
  ...
  virtual double compute_inter_detjac_inters_cubpts(int in_inter, arrayt<double> d_pos)=0;

  /*! evaluate nodal basis */
  virtual double eval_nodal_basis(int in_index, arrayt<double> in_loc)=0;

  /*! evaluate nodal basis for restart file*/
  virtual double eval_nodal_basis_restart(int in_index, arrayt<double> in_loc)=0;

  /*! evaluate derivative of nodal basis */
  virtual double eval_d_nodal_basis(int in_index, int in_cpnt, arrayt<double> in_loc)=0;

  virtual void fill_opp_3(arrayt<double>& opp_3)=0;

  /*! evaluate divergence of vcjh basis */
  //virtual double eval_div_vcjh_basis(int in_index, arrayt<double>& loc)=0;

  /*! evaluate nodal shape basis */
  virtual double eval_nodal_s_basis(int in_index, arrayt<double> in_loc, int in_n_spts)=0;

  /*! evaluate derivative of nodal shape basis */
  virtual void eval_d_nodal_s_basis(arrayt<double> &d_nodal_s_basis, arrayt<double> in_loc, int in_n_spts)=0;

  /*! Calculate SGS flux */
  void calc_sgsf_upts(arrayt<double>& temp_u, arrayt<double>& temp_grad_u, double& detjac, int ele, int upt, arrayt<double>& temp_sgsf);

  /*! rotate velocity components to surface*/
  arrayt<double> calc_rotation_matrix(arrayt<double>& norm);

  /*! calculate wall shear stress using LES wall model*/
  void calc_wall_stress(double rho, arrayt<double>& urot, double ene, double mu, double Pr, double gamma, double y, arrayt<double>& tau_wall, double q_wall);
  ...
  void CalcIntegralQuantities(int n_integral_quantities, arrayt <double>& integral_quantities);

  /*! Compute time-average diagnostic quantities */
  void CalcTimeAverageQuantities(double& time);

  void compute_wall_forces(arrayt<double>& inv_force, arrayt<double>& vis_force, double& temp_cl, double& temp_cd, ofstream& coeff_file, bool write_forces);

  arrayt<double> compute_error(int in_norm_type, double& time);
  
  arrayt<double> get_pointwise_error(arrayt<double>& sol, arrayt<double>& grad_sol, arrayt<double>& loc, double& time, int in_norm_type);

  /*! calculate position of a point in physical (dynamic) space from (r,s,t) coordinates*/
  void calc_pos_dyn(arrayt<double> in_loc, int in_ele, arrayt<double> &out_pos);
   ...
   */
  void calc_pos_dyn_fpt(int in_fpt, int in_ele, arrayt<double> &out_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_pos_dyn_upt(int in_upt, int in_ele, arrayt<double> &out_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_pos_dyn_ppt(int in_ppt, int in_ele, arrayt<double> &out_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_pos_dyn_vol_cubpt(int in_cubpt, int in_ele, arrayt<double> &out_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele, arrayt<double> &out_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn(arrayt<double> in_loc, int in_ele, arrayt<double> &out_d_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_fpt(int in_fpt, int in_ele, arrayt<double> &out_d_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_upt(int in_upt, int in_ele, arrayt<double> &out_d_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_vol_cubpt(int in_cubpt, int in_ele, arrayt<double> &out_d_pos);
   ...
   * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele, arrayt<double> &out_d_pos);
  ...
  /*! initialize arrayts for storing grid velocities */
  void initialize_grid_vel(int in_max_n_spts_per_ele);

  /*! set grid velocity on element shape points */
  void set_grid_vel_spt(int in_ele, int in_spt, arrayt<double> in_vel);
  ...
  /*! Get arrayt of grid velocity at all plot points */
  arrayt<double> get_grid_vel_ppts(void);
...
protected:
  ...
  /// flag to avoid re-setting-up transform arrayts
  bool first_time;
  ...
  */
  ...
  arrayt<double> tdA_fpts;

	/*! determinant of interface jacobian at flux points */
	arrayt< arrayt<double> > inter_detjac_inters_cubpts;
  ...
  /*!
        ...
        matrix mapping:
        */
  arrayt< arrayt<double> > disu_upts;

	/*!
	running time-averaged diagnostic fields at solution points
	*/
	arrayt<double> disu_average_upts;
  ...
  double spinup_time;
	...
	*/
	arrayt<double> disuf_upts;

  /*! position at the plot points */
  arrayt< arrayt<double> > pos_ppts;
  ...
  arrayt<double> opp_0;
  arrayt<double> opp_0_data;
  arrayt<int> opp_0_cols;
  arrayt<int> opp_0_b;
  arrayt<int> opp_0_e;
  int opp_0_sparse;

#ifdef _GPU
  arrayt<double> opp_0_ell_data;
  arrayt<int> opp_0_ell_indices;
  ...
  arrayt< arrayt<double> > opp_1;
  arrayt< arrayt<double> > opp_1_data;
  arrayt< arrayt<int> > opp_1_cols;
  arrayt< arrayt<int> > opp_1_b;
  arrayt< arrayt<int> > opp_1_e;
  int opp_1_sparse;
#ifdef _GPU
  arrayt< arrayt<double> > opp_1_ell_data;
  arrayt< arrayt<int> > opp_1_ell_indices;
  arrayt<int> opp_1_nnz_per_row;
#endif
  ...
  arrayt< arrayt<double> > opp_2;
  arrayt< arrayt<double> > opp_2_data;
  arrayt< arrayt<int> > opp_2_cols;
  arrayt< arrayt<int> > opp_2_b;
  arrayt< arrayt<int> > opp_2_e;
  int opp_2_sparse;
#ifdef _GPU
  arrayt< arrayt<double> > opp_2_ell_data;
  arrayt< arrayt<int> > opp_2_ell_indices;
  arrayt<int> opp_2_nnz_per_row;
#endif
  ...
  arrayt<double> opp_3;
  arrayt<double> opp_3_data;
  arrayt<int> opp_3_cols;
  arrayt<int> opp_3_b;
  arrayt<int> opp_3_e;
  int opp_3_sparse;
#ifdef _GPU
  arrayt<double> opp_3_ell_data;
  arrayt<int> opp_3_ell_indices;
  ...
  arrayt< arrayt<double> >  opp_4;
  arrayt< arrayt<double> >  opp_4_data;
  arrayt< arrayt<int> > opp_4_cols;
  arrayt< arrayt<int> > opp_4_b;
  arrayt< arrayt<int> > opp_4_e;
  int opp_4_sparse;
#ifdef _GPU
  arrayt< arrayt<double> > opp_4_ell_data;
  arrayt< arrayt<int> > opp_4_ell_indices;
  arrayt< int > opp_4_nnz_per_row;
#endif
  ...
  arrayt< arrayt<double> > opp_5;
  arrayt< arrayt<double> > opp_5_data;
  arrayt< arrayt<int> > opp_5_cols;
  arrayt< arrayt<int> > opp_5_b;
  arrayt< arrayt<int> > opp_5_e;
  int opp_5_sparse;
#ifdef _GPU
  arrayt< arrayt<double> > opp_5_ell_data;
  arrayt< arrayt<int> > opp_5_ell_indices;
  arrayt<int> opp_5_nnz_per_row;
#endif
  ...
  arrayt<double> opp_6;
  arrayt<double> opp_6_data;
  arrayt<int> opp_6_cols;
  arrayt<int> opp_6_b;
  arrayt<int> opp_6_e;
  int opp_6_sparse;
#ifdef _GPU
  arrayt<double> opp_6_ell_data;
  arrayt<int> opp_6_ell_indices;
  ...
  arrayt< arrayt<double> > opp_inters_cubpts;
  arrayt<double> opp_volume_cubpts;
  ...
  arrayt<double> dt_local;
  double dt_local_new;
  arrayt<double> dt_local_mpi;

  /*! Artificial Viscosity variables */
  arrayt<double> vandermonde;
  arrayt<double> inv_vandermonde;
  arrayt<double> vandermonde2D;
  arrayt<double> inv_vandermonde2D;
  arrayt<double> area_coord_upts;
  arrayt<double> area_coord_fpts;
  arrayt<double> epsilon;
  arrayt<double> epsilon_upts;
  arrayt<double> epsilon_fpts;
  arrayt<double> concentration_arrayt;
  arrayt<double> sensor;
  arrayt<double> sigma;

  arrayt<double> min_dt_local;

  /*! Global cell number of element as in the code */
  arrayt<int> ele2global_ele_code;
