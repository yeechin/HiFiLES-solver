...
#include "../data/loc_1d_gauss_lobatto_pts.dat"
  ...
  if (upts_type_pri_tri==0) // tri: inter
    {
      arrayt<double> loc_inter_pts(n_upts_tri,2);
#include "../data/loc_tri_inter_pts.dat"
    ...
    {
      arrayt<double> loc_alpha_pts(n_upts_tri,2);
...
void eles_pris::set_tloc_fpts(void)
{
  ...
  arrayt<double> loc_tri_fpts( (order+1)*(order+2)/2,2);
...
double eles_pris::compute_inter_detjac_inters_cubpts(int in_inter,arrayt<double> d_pos)
{
  double output = 0.;
  double xr, xs, xt;
  double yr, ys, yt;
...
void eles_pris::set_vandermonde_tri()
{
  ...
  // Store its inverse
  inv_vandermonde_tri = inv_arrayt(vandermonde_tri);
}

// initialize the vandermonde matrix
void eles_pris::set_vandermonde_tri_restart()
{
  arrayt<double> vandermonde_tri_rest;
  ...
  // Store its inverse
  inv_vandermonde_tri_rest = inv_arrayt(vandermonde_tri_rest);
}
...
double eles_pris::eval_nodal_basis(int in_index, arrayt<double> in_loc)
{
  ...
  arrayt<double> dubiner_basis_at_loc(n_upts_tri);
  for (int i=0;i<n_upts_tri;i++)
...
double eles_pris::eval_nodal_basis_restart(int in_index, arrayt<double> in_loc)
{
  double oned_nodal_basis_at_loc;
  double tri_nodal_basis_at_loc;
  ...
  // First evaluate the normalized Dubiner basis at position in_loc
  arrayt<double> dubiner_basis_at_loc(n_upts_tri_rest);
  for (int i=0;i<n_upts_tri_rest;i++)
...
double eles_pris::eval_d_nodal_basis(int in_index, int in_cpnt, arrayt<double> in_loc)
{
...
double eles_pris::eval_nodal_s_basis(int in_index, arrayt<double> in_loc, int in_n_spts)
{
...
void eles_pris::eval_d_nodal_s_basis(arrayt<double> &d_nodal_s_basis, arrayt<double> in_loc, int in_n_spts)
{
...
void eles_pris::fill_opp_3(arrayt<double>& opp_3)
{

  arrayt<double> loc(3);
  arrayt<double> opp_3_tri(n_upts_tri,3*(order+1));
...
double eles_pris::eval_div_vcjh_basis(int in_index, arrayt<double>& loc)
{
