...
void eles::setup(int in_n_eles, int in_max_n_spts_per_ele)
{
  ...
  if (n_eles!=0)
  {
    ...
    // Allocate extra arrayts for LES models
    if(LES) {
      ...
      // allocate dummy arrayt for passing to GPU routine
      else {
        disuf_upts.setup(1);
      }
    ...
    // Dummy arrayts to pass to GPU kernel wrapper
    else {
      disuf_upts.setup(1);
      Lu.setup(1);
      uu.setup(1);
    ...
    // Allocate arrayt for wall distance if using a RANS-based turbulence model or LES wall model
    if (run_input.turb_model > 0) {
      wall_distance.setup(n_upts_per_ele,n_eles,n_dims);
      wall_distance_mag.setup(n_upts_per_ele,n_eles);
      zero_arrayt(wall_distance);
      zero_arrayt(wall_distance_mag);
      twall.setup(1);
    }
    ...
    // Allocate SGS flux arrayt if using LES or wall model
    ...
    zero_arrayt(src_upts);

    // Allocate arrayt for grid velocity
    temp_v.setup(n_dims);
    ...
    // Set connectivity arrayt. Needed for Paraview output.
    if (ele_type==3) // prism
...
arrayt<int> eles::get_connectivity_plot()
{
...
void eles::set_ics(double& time)
{
  ...
  arrayt<double> pos(n_dims);
  arrayt<double> ics(n_fields);
  
  arrayt<double> grad_rho(n_dims);
  ...
  for(i=0;i<n_eles;i++)
  {
    for(j=0;j<n_upts_per_ele;j++)
    {
      ...
      if(run_input.perturb_ic==1 and n_dims==3)
      {
        ...
        L_y = pi_t;
        L_z = 2.;
        ics(3) += alpha*exp(-pow((pos(0)-L_x/2.)/L_x,2))*exp(-pow(pos(1)/L_y,2))*cos(4.*pi_t*pos(2)/L_z);
      }
...
void eles::write_restart_data(ofstream& restart_file)
{
  ...
  restart_file << n_eles << endl;
  restart_file << "ele2global_ele arrayt" << endl;
  for (int i=0;i<n_eles;i++)
...
void eles::mv_all_cpu_gpu(void)
{
  ...
  if (n_eles!=0)
  {
    ...
    // Grid Velocity-related arrayts for moving meshes
    vel_spts.cp_cpu_gpu();
    ...
    if(run_input.ArtifOn){
        ...
        if(run_input.artif_type == 1)
        {
          concentration_arrayt.mv_cpu_gpu();
          sigma.mv_cpu_gpu();
        }
...
// move wall distance arrayt to gpu
...
// move wall distance magnitude arrayt to gpu
...
void eles::extrapolate_solution(int in_disu_upts_from)
{
  if (n_eles!=0) {
    ...
    else if(opp_0_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::extrapolate_totalFlux()
{
  if (n_eles!=0)
  {
    ...
    else if(opp_1_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::calculate_divergence(int in_div_tconf_upts_to)
{
  if (n_eles!=0)
  {
    ...
    else if(opp_2_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::calculate_corrected_divergence(int in_div_tconf_upts_to)
{
  if (n_eles!=0)
  {
    ...
    else if(opp_3_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::calculate_gradient(int in_disu_upts_from)
{
  if (n_eles!=0)
  {
    ...
    else if(opp_4_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::correct_gradient(void)
{
  if (n_eles!=0)
  {
    ...
    else if(opp_5_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::extrapolate_corrected_gradient(void)
{
  if (n_eles!=0)
  {
    ...
    else if(opp_6_sparse==1) // mkl blas four-arrayt csr format
    {
#if defined _MKL_BLAS
...
void eles::calc_sgs_terms(int in_disu_upts_from)
{
  if (n_eles!=0) {
    ...
    /*! If Similarity model, compute product terms and Leonard tensors */
      ...
      /*! third dimension of Lu, uu arrayts */
      if(n_dims==2)      dim3 = 3;
      else if(n_dims==3) dim3 = 6;
      
      /*! Calculate velocity and energy product arrayts uu, ue */
      for(i=0;i<n_upts_per_ele;i++) {
        for(j=0;j<n_eles;j++) {
          for(k=0;k<n_fields;k++) {
          ...
          /*! note that product arrayts are symmetric */
          if(n_dims==2) {
  ...
  if (n_eles!=0)
  {
    ...
    for(i=0;i<n_eles;i++) {
      ...
      for(j=0;j<n_upts_per_ele;j++)
      {
        ...
        // If LES, add SGS flux to global arrayt (needed for interface flux calc)
        if(LES > 0) {
          ...
          if (motion) {
            ...
            // Copy back to original flux arrayt
            for (k=0; k<n_fields; k++) {
              for(l=0; l<n_dims; l++) {
...
void eles::calc_sgsf_upts(arrayt<double>& temp_u, arrayt<double>& temp_grad_u, double& detjac, int ele, int upt, arrayt<double>& temp_sgsf)
{
  int i,j,k;
  int eddy, sim, wall;
  double Cs;
  ...
  arrayt<double> u(n_dims);
  arrayt<double> drho(n_dims), dene(n_dims), dke(n_dims), de(n_dims);
  arrayt<double> dmom(n_dims,n_dims), du(n_dims,n_dims), S(n_dims,n_dims);
  
  // quantities for wall model
  arrayt<double> norm(n_dims);
  arrayt<double> tau(n_dims,n_dims);
  arrayt<double> Mrot(n_dims,n_dims);
  arrayt<double> temp(n_dims,n_dims);
  arrayt<double> urot(n_dims);
  arrayt<double> tw(n_dims);
  ...
  // Initialize SGS flux arrayt to zero
  zero_arrayt(temp_sgsf);
  ...
  if(wall) {
    ...
    // populate ndims*ndims rotated stress arrayt
    zero_arrayt(tau);
    
    for(i=0;i<n_dims-1;i++) tau(i+1,0) = tau(0,i+1) = tw(i);
    
    // rotate stress arrayt back to Cartesian coordinates
    zero_arrayt(temp);
    for(i=0;i<n_dims;++i)
      for(j=0;j<n_dims;++j)
        for(k=0;k<n_dims;++k)
  ...
  // Free-stream SGS flux
    ...
    if(eddy==1) {
      ...
      //  Typically Cw = 0.5.
        ...
        double denom=0.0;
        double eps=1.e-12;
        arrayt<double> Sq(n_dims,n_dims);
        diag = 0.0;
...
void eles::calc_wall_distance(int n_seg_noslip_inters, int n_tri_noslip_inters, int n_quad_noslip_inters, arrayt< arrayt<double> > loc_noslip_bdy)
{
  if(n_eles!=0)
  {
    int i,j,k,m,n,p;
    ...
    double distmin;
    arrayt<double> pos(n_dims);
    arrayt<double> pos_bdy(n_dims);
    arrayt<double> vec(n_dims);
    arrayt<double> vecmin(n_dims);
...
void eles::calc_wall_distance_parallel(arrayt<int> n_seg_inters_arrayt, arrayt<int> n_tri_inters_arrayt, arrayt<int> n_quad_inters_arrayt, arrayt< arrayt<double> > loc_noslip_bdy_global, int nproc)
{
  if(n_eles!=0)
  {
    int i,j,k,m,n,p;
    ...
    for (i=0;i<n_eles;++i) {
      for (j=0;j<n_upts_per_ele;++j) {
        ...
        for (p=0;p<nproc;++p) {
          ...
          // line segment boundaries
          for (k=0;k<n_seg_inters_arrayt(p);++k) {
          ...
          // tri boundaries
          for (k=0;k<n_tri_inters_arrayt(p);++k) {
          ...
          // quad boundaries
          for (k=0;k<n_quad_inters_arrayt(p);++k) {
...
arrayt<double> eles::calc_rotation_matrix(arrayt<double>& norm)
{
  arrayt <double> mrot(n_dims,n_dims);
  double nn;
...
void eles::calc_wall_stress(double rho, arrayt<double>& urot, double ene, double mu, double Pr, double gamma, double y, arrayt<double>& tau_wall, double q_wall)
{
  double eps = 1.e-10;
          ...
          tmp_sensor = 0;
            for(int i=0; i<in_order+1; i++)
            {
                ...
                for(int j=0; j<in_order+1; j++){
                    uE[j] = 0;
                    for(int k=0; k<in_order+1; k++)
                        uE[j] += modal_rho[k]*concentration_arrayt_ptr[j*(in_order+1) + k];
...
void eles::set_shape_node(int in_spt, int in_ele, arrayt<double>& in_pos)
{
  for(int i=0;i<n_dims;i++)
  {
...
void eles::set_dynamic_shape_node(int in_spt, int in_ele, arrayt<double> &in_pos)
{
  for(int i=0;i<n_dims;i++) {
...
#ifdef _CPU
    arrayt_to_mklcsr(opp_0,opp_0_data,opp_0_cols,opp_0_b,opp_0_e);
#endif
    
#ifdef _GPU
    arrayt_to_ellpack(opp_0, opp_0_ell_data, opp_0_ell_indices, opp_0_nnz_per_row);
...
void eles::set_opp_1(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      arrayt_to_mklcsr(opp_1(i),opp_1_data(i),opp_1_cols(i),opp_1_b(i),opp_1_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      arrayt_to_ellpack(opp_1(i), opp_1_ell_data(i), opp_1_ell_indices(i), opp_1_nnz_per_row(i));
...
void eles::set_opp_2(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      arrayt_to_mklcsr(opp_2(i),opp_2_data(i),opp_2_cols(i),opp_2_b(i),opp_2_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      arrayt_to_ellpack(opp_2(i), opp_2_ell_data(i), opp_2_ell_indices(i), opp_2_nnz_per_row(i));
...
#ifdef _CPU
    arrayt_to_mklcsr(opp_3,opp_3_data,opp_3_cols,opp_3_b,opp_3_e);
#endif
    
#ifdef _GPU
    arrayt_to_ellpack(opp_3, opp_3_ell_data, opp_3_ell_indices, opp_3_nnz_per_row);
...
void eles::set_opp_4(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
    ...
    for (int i=0;i<n_dims;i++)
    {
      arrayt_to_mklcsr(opp_4(i),opp_4_data(i),opp_4_cols(i),opp_4_b(i),opp_4_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      arrayt_to_ellpack(opp_4(i), opp_4_ell_data(i), opp_4_ell_indices(i), opp_4_nnz_per_row(i));
...
void eles::set_opp_5(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      arrayt_to_mklcsr(opp_5(i),opp_5_data(i),opp_5_cols(i),opp_5_b(i),opp_5_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      arrayt_to_ellpack(opp_5(i), opp_5_ell_data(i), opp_5_ell_indices(i), opp_5_nnz_per_row(i));
...
#ifdef _CPU
    arrayt_to_mklcsr(opp_6,opp_6_data,opp_6_cols,opp_6_b,opp_6_e);
#endif
    
#ifdef _GPU
    arrayt_to_ellpack(opp_6, opp_6_ell_data, opp_6_ell_indices, opp_6_nnz_per_row);
...
void eles::calc_pos_ppts(int in_ele, arrayt<double>& out_pos_ppts)
{
  int i,j;
...
void eles::calc_disu_ppts(int in_ele, arrayt<double>& out_disu_ppts)
{
  if (n_eles!=0)
  {
    ...
    arrayt<double> disu_upts_plot(n_upts_per_ele,n_fields);
...
void eles::calc_grad_disu_ppts(int in_ele, arrayt<double>& out_grad_disu_ppts)
{
  if (n_eles!=0)
  {
    ...
    arrayt<double> grad_disu_upts_temp(n_upts_per_ele,n_fields,n_dims);
...
void eles::calc_time_average_ppts(int in_ele, arrayt<double>& out_disu_average_ppts)
{
  if (n_eles!=0)
  {
    ...
    arrayt<double> disu_average_upts_plot(n_upts_per_ele,n_average_fields);
...
void eles::calc_sensor_ppts(int in_ele, arrayt<double>& out_sensor_ppts)
{
    if (n_eles!=0)
    {
...
// calculate solution at the plot points
void eles::calc_epsilon_ppts(int in_ele, arrayt<double>& out_epsilon_ppts)
{
  if (n_eles!=0)
  {
      ...
      arrayt<double> epsilon_upts_plot(n_upts_per_ele);
...
void eles::calc_diagnostic_fields_ppts(int in_ele, arrayt<double>& in_disu_ppts, arrayt<double>& in_grad_disu_ppts, arrayt<double>& in_sensor_ppts, arrayt<double>& in_epsilon_ppts, arrayt<double>& out_diag_field_ppts, double& time)
{
  int i,j,k,m;
  ...
  for(j=0;j<n_ppts_per_ele;j++)
  {
    ...
    for (k=0;k<n_diagnostic_fields;k++)
    {
      ...
      // set arrayt with solution point value
...
void eles::calc_pos_upt(int in_upt, int in_ele, arrayt<double>& out_pos)
{
  int i;
...
void eles::set_transforms(void)
{
  if (n_eles!=0)
  {
    ...
    arrayt<double> loc(n_dims);
    arrayt<double> pos(n_dims);
    arrayt<double> d_pos(n_dims,n_dims);
    arrayt<double> tnorm_dot_inv_detjac_mul_jac(n_dims);
...
void eles::set_transforms_dynamic(void)
{
  ...
  if (n_eles!=0 && motion)
  {
    ...
    arrayt<double> pos(n_dims);
    arrayt<double> d_pos(n_dims,n_dims);
    arrayt<double> norm_dot_JGinv(n_dims);  // un-normalized normal vector in moving-physical domain
...
//          }
    ...
    // To avoid re-setting up ALL transform arrayts in the future (for dynamic grids)
    first_time = false;
  }
#endif
...
double* eles::get_normal_disu_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter, int in_field, int in_ele, arrayt<double> temp_loc, double temp_pos[3])
{
...
void eles::calc_pos(arrayt<double> in_loc, int in_ele, arrayt<double>& out_pos)
{
  int i,j;
...
void eles::calc_pos_dyn(arrayt<double> in_loc, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_pos_dyn_fpt(int in_fpt, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_pos_dyn_upt(int in_upt, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_pos_dyn_ppt(int in_ppt, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_pos_dyn_vol_cubpt(int in_ppt, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele, arrayt<double>& out_pos)
{
    int i,j;
...
void eles::calc_d_pos(arrayt<double> in_loc, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dxi_j
 */
void eles::calc_d_pos_upt(int in_upt, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dxi_j
 */
void eles::calc_d_pos_fpt(int in_fpt, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
 ...
 * \param[in] in_ele - local element ID
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn(arrayt<double> in_loc, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_fpt(int in_fpt, int in_ele, arrayt<double>& out_d_pos)
{
  if (run_input.motion==4) {
    // Analytical formula for perturbed motion
    out_d_pos(0,0) = 1 + 0.2*pi_t*cos(pi_t*pos_fpts(in_fpt,in_ele,0)/10)*sin(pi_t*pos_fpts(in_fpt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(0,1) = 0.2*pi_t*sin(pi_t*pos_fpts(in_fpt,in_ele,0)/10)*cos(pi_t*pos_fpts(in_fpt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(1,0) = 0.2*pi_t*cos(pi_t*pos_fpts(in_fpt,in_ele,0)/10)*sin(pi_t*pos_fpts(in_fpt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(1,1) = 1 + 0.2*pi_t*sin(pi_t*pos_fpts(in_fpt,in_ele,0)/10)*cos(pi_t*pos_fpts(in_fpt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
  }
  else
  {
    ...
    // Calculate dx/dr
    arrayt<double> dxdr(n_dims,n_dims);
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_upt(int in_upt, int in_ele, arrayt<double>& out_d_pos)
{
  if (run_input.motion==4) {
    // Analytical formula for perturbed motion test case
    out_d_pos(0,0) = 1 + 0.2*pi_t*cos(pi_t*pos_upts(in_upt,in_ele,0)/10)*sin(pi_t*pos_upts(in_upt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(0,1) = 0.2*pi_t*sin(pi_t*pos_upts(in_upt,in_ele,0)/10)*cos(pi_t*pos_upts(in_upt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(1,0) = 0.2*pi_t*cos(pi_t*pos_upts(in_upt,in_ele,0)/10)*sin(pi_t*pos_upts(in_upt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
    out_d_pos(1,1) = 1 + 0.2*pi_t*sin(pi_t*pos_upts(in_upt,in_ele,0)/10)*cos(pi_t*pos_upts(in_upt,in_ele,1)/10)*sin(2*pi_t*run_input.rk_time/10);
  }
  else
  {
    // For all cases which do not have an analytical solution (i.e. for linear-elasticity deformation)
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_vol_cubpt(int in_cubpt, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
 ...
 * \param[out] out_d_pos - arrayt of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele, arrayt<double>& out_d_pos)
{
  int i,j,k;
...
arrayt<double> eles::compute_error(int in_norm_type, double& time)
{
  arrayt<double> disu_cubpt(n_fields);
  arrayt<double> grad_disu_cubpt(n_fields,n_dims);
  double detjac;
  arrayt<double> pos(n_dims);
  
  arrayt<double> error(2,n_fields);  //storage
  arrayt<double> error_sum(2,n_fields);  //output
...
arrayt<double> eles::get_pointwise_error(arrayt<double>& sol, arrayt<double>& grad_sol, arrayt<double>& loc, double& time, int in_norm_type)
{
  arrayt<double> error(2,n_fields);  //output
  
  arrayt<double> error_sol(n_fields);
  arrayt<double> error_grad_sol(n_fields,n_dims);
...
//#ifdef _CPU
  ...
  if (n_eles!=0) {
    ...
    arrayt <int> inflowinters(n_bdy_eles,n_inters_per_ele);
    arrayt <double> body_force(n_fields);
    arrayt <double> disu_cubpt(4);
    arrayt <double> integral(4);
    arrayt <double> norm(n_dims), flow(n_dims), loc(n_dims), pos(n_dims);
    ofstream write_mdot;
    bool open_mdot;
    ...
    arrayt<double> integral_global(4);
    for (m=0;m<4;m++)
    {
      integral_global(m) = 0.;
...
void eles::CalcIntegralQuantities(int n_integral_quantities, arrayt <double>& integral_quantities)
{
  arrayt<double> disu_cubpt(n_fields);
  arrayt<double> grad_disu_cubpt(n_fields,n_dims);
  arrayt<double> S(n_dims,n_dims);
  double wx, wy, wz;
  double dudx, dudy, dudz;
  double dvdx, dvdy, dvdz;
  double dwdx, dwdy, dwdz;
...
void eles::compute_wall_forces( arrayt<double>& inv_force, arrayt<double>& vis_force,  double& temp_cl, double& temp_cd, ofstream& coeff_file, bool write_forces)
{
  
  arrayt<double> u_l(n_fields),norm(n_dims);
  double p_l,v_sq,vn_l;
  arrayt<double> grad_u_l(n_fields,n_dims);
  arrayt<double> dv(n_dims,n_dims);
  arrayt<double> de(n_dims);
  arrayt<double> drho(n_dims);
  arrayt<double> taun(n_dims);
  arrayt<double> tautan(n_dims);
  arrayt<double> Finv(n_dims);
  arrayt<double> Fvis(n_dims);
  arrayt<double> loc(n_dims);
  arrayt<double> pos(n_dims);
 ...
 *  TODO: CUDA */
void eles::set_grid_vel_spt(int in_ele, int in_spt, arrayt<double> in_vel)
{
  for (int i=0; i<n_dims; i++)
...
/*! Setup arrayts for storing grid velocity */
...
arrayt<double> eles::get_grid_vel_ppts(void)
{
  return vel_ppts;
}
